...
export default function RegisterPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/auth/register', {
      ...
      body: JSON.stringify({ name, phone, password, inviteCode }),
    })
    ...
    if (!res.ok) {
      ...
      setErr(j.error ?? 'Erro no cadastro')
      return
    }
  ...
  return (
    <div className="space-y-4">
      ...
      <form onSubmit={onSubmit} className="space-y-3 rounded-lg border border-zinc-800 p-4">
        ...
        <div className="space-y-1">
          <label className="text-sm text-zinc-300">Código do bolão</label>
          <input
            className="w-full rounded-md border border-zinc-700 bg-zinc-950 px-3 py-2"
            value={inviteCode}
